 ...
 *     security:
 *       - bearerAuth: []
 *     summary: Create a new Logistic Regression phenotype
 *     description: Create a phenotype definition based on the Logistic Regression technique (using the scikit-learn implementation: https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html).
 *     requestBody:
 *       required: true
 *       content:
 *         multipart/form-data:
...
router.post('/addPhenotype', jwt({secret:config.get("jwt.RSA_PRIVATE_KEY"), algorithms:['RS256']}), async function(req, res, next) {
    ...
    source_implementation_file_path = "templates/LogisticRegression/" + implementation_file_name
 ...
 *         type: string
 *         required: true
 *         description: Name of the existing Logistic Regression phenotype
 *       - in: path
...
router.get("/generate/:workflowName/:trainDatasetName/:testDatasetName", jwt({secret:config.get("jwt.RSA_PRIVATE_KEY"), algorithms:['RS256']}), async function(req, res, next) {
    ...
    //   ==> USERS MUST NOT USE HERE A PHENOTYPE THAT IS NOT OF LOGISTIC REGRESSION TYPE.
    try { 
    ...
    templates_folder_path = "templates/LogisticRegression/"
    ...
    try {
        generator_url = config.get("generator.URL") + "/LogisticRegression/getStepCwl/1"
        ...
        generator_url = config.get("generator.URL") + "/LogisticRegression/getStepCwl/2"
        ...
        generator_url = config.get("generator.URL") + "/LogisticRegression/getStepCwl/3"
        ...
        generator_url = config.get("generator.URL") + "/LogisticRegression/getStepCwl/4"
    } catch(error) {
    ...
    try {
        generator_url = config.get("generator.URL") + "/LogisticRegression/getMainCwl"
