...
/**
 * @swagger
 * /phenoflowml/SVC/addPhenotype:
 *   post:
 *     security:
 *       - bearerAuth: []
 *     summary: Create a new Support Vector Classification phenotype
 *     description: Create a phenotype definition based on the Support Vector Classification technique (using the scikit-learn implementation).
 *     requestBody:
 *       required: true
 *       content:
 *         multipart/form-data:
...
router.post('/addPhenotype', jwt({secret:config.get("jwt.RSA_PRIVATE_KEY"), algorithms:['RS256']}), async function(req, res, next) {
    ...
    source_implementation_file_path = "templates/SVC/" + implementation_file_name
...
/**
 * @swagger
 * /phenoflowml/SVC/uploadCsvDataset:
 *   post:
 *     security:
 *       - bearerAuth: []
...
/**
 * @swagger
 * /phenoflowml/SVC/generate/{workflowName}/{trainDatasetName}/{testDatasetName}:
 *   get:
 *     summary: Generate a Support Vector Classification phenotype
 *     description: Generate a phenotype based on the Support Vector Classification technique, indicanting and existing workflow/phenotype name and an existing train and test dataset names (including the extension)
 *     parameters:
 *       - in: path
 *         name: workflowName
 *         type: string
 *         required: true
 *         description: Name of the existing Support Vector Classification phenotype
 *       - in: path
...
router.get("/generate/:workflowName/:trainDatasetName/:testDatasetName", jwt({secret:config.get("jwt.RSA_PRIVATE_KEY"), algorithms:['RS256']}), async function(req, res, next) {
    ...
    //   ==> USERS MUST NOT USE HERE A PHENOTYPE THAT IS NOT OF SUPPORT VECTOR CLASSIFICATION TYPE.
    try { 
    ...
    templates_folder_path = "templates/SVC/"
    ...
    try {
        generator_url = config.get("generator.URL") + "/SVC/getStepCwl/1"
        ...
        generator_url = config.get("generator.URL") + "/SVC/getStepCwl/2"
        ...
        generator_url = config.get("generator.URL") + "/SVC/getStepCwl/3"
        ...
        generator_url = config.get("generator.URL") + "/SVC/getStepCwl/4"
    } catch(error) {
    ...
    try {
        generator_url = config.get("generator.URL") + "/SVC/getMainCwl"
    ...
    try {
        generator_url = config.get("generator.URL") + "/SVC/generateMainYml/" + req.params.trainDatasetName + "/" + req.params.testDatasetName
